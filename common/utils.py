...
def compute_accuracy(logits, labels, qry_num):
    accuracy = (logits == labels).astype(float).sum() / qry_num * 100
    return accuracy


def cp_accuracy(logits, labels):
...
def parse_args(arg_mode):
    parser = argparse.ArgumentParser(description='Multi-constraint meta-transfer Learning for few-shot Image Classification')

    ''' about dataset '''
    parser.add_argument('-dataset', type=str, default='miniimagenet',
                        choices=['miniimagenet', 'cub', 'tieredimagenet', 'cifar_fs'])
    parser.add_argument('-data_dir', type=str, default='/media/breeze/dataset', help='dir of datasets')
    ...
    parser.add_argument('-lr', type=float, default=0.1, help='learning rate0.1')
    ...
    parser.add_argument('-test_episode', type=int, default=500, help='500 number of testing episodes after training')
    parser.add_argument('-pre_pth', type=str, default='mini_5shot', help='pre training model')
    ...
    ''' about env '''
    parser.add_argument('-gpu', default='0,1', help='the GPU ids e.g. \"0\", \"0,1\", \"0,1,2\", etc')
    parser.add_argument('-extra_dir', type=str, default='your_run', help='extra dir name added to checkpoint dir')
